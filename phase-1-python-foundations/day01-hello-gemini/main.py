...
# ── Clients ────────────────────────────────────────────────────────────────────
# We load credentials dynamically from environment variables.
# This prevents code failures if the user is missing credentials for one of the APIs.
# Conditional initialization checks if keys exist, assigning None to models otherwise.
...
def call_gemini(prompt: str) -> dict:
    # We use time.perf_counter() for high-resolution timing to capture request round-trip latency.
    t0 = time.perf_counter()
    resp = gemini_model.generate_content(prompt)
    latency = time.perf_counter() - t0
    
    # Extracting total tokens generated by Google's API model
    return {
        "text": resp.text,
...
def call_openai(prompt: str) -> dict:
    # We use time.perf_counter() to benchmark OpenAI round-trip network performance.
    t0 = time.perf_counter()
    ...
    # Extracting total tokens used (prompt + completion) from OpenAI API response
    return {
